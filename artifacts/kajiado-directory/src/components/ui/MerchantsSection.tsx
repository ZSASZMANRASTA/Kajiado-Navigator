    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      {!selectedTown ? (
        <div className="mb-8">
          <div className="flex items-start justify-between gap-4 mb-6">
            <div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-800 leading-tight">
                Kajiado's Best<br /><span className="text-ochre">Local Businesses</span>
              </h2>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
            <button onClick={onClearTown}
              className={`shrink-0 flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border transition-all ${!selectedTown ? "bg-gray-800 text-white border-gray-800" : "bg-white text-gray-500 border-gray-200"}`}>
              All Towns
            </button>
            {towns.map(town => {
          ...
          <button onClick={onMapOpen}
            ...
            <div className="h-28 w-full relative" style={{ background: "linear-gradient(135deg, #2d4a2d 0%, #3a5c3a 20%, #2a4020 40%, #4a6830 60%, #3d5528 80%, #2d4a2d 100%)" }}>
              <div className="absolute inset-0 opacity-20" style={{ backgroundImage: "repeating-linear-gradient(0deg,transparent,transparent 20px,rgba(255,255,255,.1) 20px,rgba(255,255,255,.1) 21px),repeating-linear-gradient(90deg,transparent,transparent 20px,rgba(255,255,255,.1) 20px,rgba(255,255,255,.1) 21px)" }} />
              {towns.map((town, i) => (
                <div key={town.id} className="absolute flex flex-col items-center" style={{ left: `${15 + i * 17}%`, top: `${20 + (i % 2) * 35}%` }}>
                  <div className="w-4 h-4 rounded-full bg-ochre border-2 border-white shadow-md flex items-center justify-center"><div className="w-1.5 h-1.5 rounded-full bg-white" /></div>
                  <span className="text-[8px] text-white font-bold mt-0.5 drop-shadow-md whitespace-nowrap">{town.name}</span>
                </div>
              ))}
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 backdrop-blur-sm text-gray-800 text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 shadow-lg">
                  <Map className="w-3.5 h-3.5 text-acacia" />Open Satellite Map
                </div>
              </div>
            </div>
            <div className="px-4 py-2.5 bg-white flex items-center justify-between">
              <div className="flex items-center gap-2"><Map className="w-3.5 h-3.5 text-acacia" /><span className="text-xs font-semibold text-gray-700">Kajiado County Satellite Map</span></div>
      ...
      <div className="mb-5 space-y-3">
        <div className="relative">
          ...
          <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search merchants…"
          ...
          {query && <button onClick={() => setQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"><X className="w-4 h-4" /></button>}
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <SlidersHorizontal className="w-3.5 h-3.5 text-gray-400 shrink-0" />
          <button onClick={() => setActiveCategory(null)}
            className={`shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full border transition-all ${!activeCategory ? "bg-gray-800 text-white border-gray-800" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}>All</button>
          {ALL_CATEGORIES.map(cat => {
            const isActive = activeCategory === cat;
            return (
              <button key={cat} onClick={() => setActiveCategory(isActive ? null : cat)}
                className={`shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full border transition-all ${isActive ? (CATEGORY_COLORS[cat] ?? "bg-gray-100 text-gray-600 border-gray-200") : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}>
                {cat}
              </button>
            );
          })}
      ...
      {(isFiltering || selectedTown) && (
        <p className="text-xs text-gray-400 mb-4">
          ...
          {isFiltering && <button onClick={() => { setQuery(""); setActiveCategory(null); }} className="ml-2 text-ochre hover:underline">Clear filters</button>}
        </p>
      )}

      {displayShops.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center mb-4"><Store className="w-6 h-6 text-gray-400" /></div>
          <p className="font-semibold text-gray-600 mb-1">No merchants found</p>
          <p className="text-sm text-gray-400">{isFiltering ? "Try a different search or category." : "No merchants listed yet."}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {displayShops.map(shop => <MerchantCard key={shop.id} shop={shop} />)}
        </div>
      )}
    </div>
  );
